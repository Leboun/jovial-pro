  ...
  const loadReminders = useCallback(async () => {
    ...
    const clean = (data as any[] | null)
      ?.map((row) => {
        ...
        return { id: Number(row?.id), status: "going", event: { id: Number(event.id), title: String(event.title ?? ""), starts_at: String(event.starts_at), cover_url: event.cover_url ?? null } } as RsvpEvent;
      })
    ...
    <View style={styles.screen}>
      ...
      {loading ? (
        ...
        <View style={styles.emptyWrap}>
          <Ionicons name="alarm-outline" size={48} color={Pastel.border} />
          <Text style={styles.emptyTitle}>Aucun rappel</Text>
          <Text style={styles.emptyText}>Les événements auxquels tu vas participer apparaîtront ici.</Text>
        </View>
      ) : (
          ...
          {items.map((item) => (
            <Pressable key={item.id} style={styles.card} onPress={() => router.push(`/event/${item.event.id}` as any)}>
              ...
              <View style={styles.badge}>
                <Text style={styles.badgeText}>J'y vais</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  badge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999, backgroundColor: Pastel.primary },
  badgeText: { fontSize: 11, fontFamily: Font.bold, color: "#FFFFFF" },
});
