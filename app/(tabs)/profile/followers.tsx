...
export default function FollowersScreen() {
  ...
  const loadFollowers = useCallback(async () => {
    ...
    if (error || !followRows?.length) { setFollowers([]); setLoading(false); return; }

    const ids = followRows.map((r: { follower_id: string }) => r.follower_id).filter(Boolean);
    ...
    setFollowers((profiles as ProfileLite[] | null) ?? []);
    setLoading(false);
  }, [userId]);
    ...
    <View style={styles.screen}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View>
          ...
          <Text style={styles.subtitle}>{followers.length} personne{followers.length !== 1 ? "s" : ""} te suivent</Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyWrap}>
          <Ionicons name="people-outline" size={48} color={Pastel.border} />
          <Text style={styles.emptyTitle}>Aucun follower</Text>
          <Text style={styles.emptyText}>Partage ton profil pour que des amis te trouvent.</Text>
        </View>
      ) : (
